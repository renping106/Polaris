...
{
    private readonly JsonSerializerOptions _jsonSerializerOptions = new(JsonSerializerDefaults.Web) { WriteIndented = true };
    private readonly List<IPlugInDescriptor> _plugInDescriptors = [];
    ...
    public async Task RefreshPlugInAsync(IPlugInDescriptor installedPlugIn)
    {
        try
        {
            ...
            var (Success, Message) = await _webAppShell.UpdateShell();

            if (Success && _shellServiceProvider.ServiceProvider != null)
            {
                var dbContextUpdator = _shellServiceProvider.ServiceProvider.GetRequiredService<IDbContextUpdater>();
                await dbContextUpdator.UpdateAsync(new DbContextChangedEvent()
                {
    ...
    private ReadOnlyCollection<IPlugInDescriptor> LoadState()
    {
        var filePath = Path.Combine(AppContext.BaseDirectory, settingFileName);
        var plugInStates = new List<IPlugInDescriptor>();
        ...
        return new ReadOnlyCollection<IPlugInDescriptor>(plugInStates);
    }

    private void SaveState()
    {
        var filePath = Path.Combine(AppContext.BaseDirectory, settingFileName);
        var jsonString = JsonSerializer.Serialize(_plugInDescriptors, _jsonSerializerOptions);
