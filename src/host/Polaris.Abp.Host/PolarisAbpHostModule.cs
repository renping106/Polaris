...
using Polaris.Abp.DatabaseManagement;
using Polaris.Abp.DatabaseManagement.Extensions;
using Polaris.Abp.Host.Data;
using Polaris.Abp.Host.Localization;
using Polaris.Abp.Host.Menus;
using Polaris.Abp.NewFireTheme;
...
[DependsOn(
    ...
    typeof(PolarisNewFireThemeModule),
    typeof(AbpAspNetCoreMvcUiLeptonXLiteThemeModule)
)]
public class PolarisAbpHostModule : AbpModule
{
    ...
    private void ConfigureVirtualFiles(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<PolarisAbpHostModule>();
            if (hostingEnvironment.IsDevelopment())
            {
                /* Using physical files in development, so we don't need to recompile on changes */
                options.FileSets.ReplaceEmbeddedByPhysical<PolarisAbpHostModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(PolarisAbpHostModule).Assembly);
        });
    }
    ...
    {
        context.Services.AddAutoMapperObjectMapper<PolarisAbpHostModule>();
        Configure<AbpAutoMapperOptions>(options =>
        {
             ...
             */
            options.AddMaps<PolarisAbpHostModule>(/* validate: true */);
        });
    }
