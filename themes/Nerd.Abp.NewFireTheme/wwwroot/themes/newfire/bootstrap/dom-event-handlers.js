    ...
    abp.dom.initializers.initializeAutocompleteSelects = function ($autocompleteSelects) {
        if ($autocompleteSelects.length) {
            $autocompleteSelects.each(function () {
                ...
                // error name: Tenant.ExtraProperties[DatabaseProvider]_Text
                // correct name: Tenant.ExtraProperties[DatabaseProvider_Text]
                let regex = /(?<=\[).+?(?=\])/;
                let extraPropertyName = name.match(regex);
                let selectedTextInputName = "";
                if (extraPropertyName) {
                    selectedTextInputName = name.replace(extraPropertyName, extraPropertyName + "_Text");
                }
                else {
                    selectedTextInputName = name + "_Text";
                }
